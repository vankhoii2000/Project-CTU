<?php
    ...
    WHERE DH.ma_khach_hang = KH.ma_khach_hang and TT.ma_don_hang = DH.ma_don_hang and TT.trang_thai = 0 and DH.ma_don_hang = {$id}";
    $donhang = mysqli_query($connect,$sql);

    $sql1 = "SELECT * FROM CHI_TIET_DON_HANG as CTDH, SAN_PHAM as SP, TRANG_THAI as TT
    WHERE CTDH.ma_san_pham = SP.ma_san_pham and CTDH.ma_don_hang = TT.ma_don_hang and TT.trang_thai = 0 and CTDH.ma_don_hang = {$id}";
    $sanpham = mysqli_query($connect, $sql1);

    if(isset($_POST['submit'])){
        $madonhang = $_GET['id'];

        $rawdate = $_POST['ngay_giao'];
        $ngaygiao = date('Y-m-d', strtotime($rawdate));
        $sql2 = "UPDATE `chi_tiet_don_hang` SET `ngay_giao`='$ngaygiao' WHERE ma_don_hang = {$madonhang}";
        $ngaygiaohang = mysqli_query($connect,$sql2);

        $sql3 = "UPDATE `trang_thai` SET `trang_thai`= '1' WHERE ma_don_hang = {$madonhang}";
        $trangthai = mysqli_query($connect,$sql3);

        header("Location: ./dangcho.php");
        exit();
    }
    ...
    <div class="container-fluid">
        ...
        <div class="container mt-5">
            ...
            <div class="container mt-5">
                <center>
                    <div>
                        <form action="#" method="POST">
                            <label for="birthday">Ngày giao hàng:</label>
                ...
                <center>
                    <a href="dangcho.php" type="button" class="btn btn-primary mt-3">Trở về</a>
                </center>
            </div>
        </div>
    </div>
